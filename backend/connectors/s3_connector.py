...
def s3_connector(bucket: str, key: str, file_type: str = "csv",
    ...
    try:
        # Per-connection credentials (passed from the saved S3 connection)
        # take priority, like Snowflake/Postgres — falling back to the
        # global .env AWS keys only when none were provided, so older
        # pipelines created before this change keep working.
        s3 = boto3.client(
            "s3",
