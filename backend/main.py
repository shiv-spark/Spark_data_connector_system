...
def get_table_data(
    ...
    filter_col: Optional[str] = Query(None),
    filter_val: Optional[str] = Query(None),
    filter_exact: bool        = Query(False, description="Exact match instead of contains/substring"),
):
    conn   = get_conn()
    cursor = conn.cursor()
    ...
    try:
        ...
        if filter_col:
            resolved_filter_col = columns_by_lower.get(filter_col.lower())
            if not resolved_filter_col:
                raise HTTPException(status_code=400, detail=f"filter_col '{filter_col}' not found in table.")
            filter_col = resolved_filter_col

        # ── 3. Build base query with optional filter ───────────────────
        #    filter_val is passed as a parameter — never interpolated.
        #    filter_exact=False (default): CAST(col AS TEXT) ILIKE '%val%'  (contains, case-insensitive)
        #    filter_exact=True:            CAST(col AS TEXT) ILIKE  'val'   (exact, case-insensitive)
        #    The default 'contains' mode can match unrelated values that share
        #    a substring (e.g. filtering 'active' also matches 'inactive') —
        #    use filter_exact=true to avoid that.
        where_clause = sql.SQL("")
        count_params = []
        data_params  = []
        ...
        if filter_col and filter_val is not None:
            ...
            like_val     = filter_val if filter_exact else f"%{filter_val}%"
            count_params = [like_val]
            data_params  = [like_val]
