...
#     # `column` isn't guaranteed to actually be a timestamp type in the DB —
#     # plenty of real tables store dates as text/varchar. Without an explicit
#     # cast, `now() - MAX(column)` fails with e.g. "operator does not exist:
#     # timestamp with time zone - text" the moment the column isn't already a
#     # native timestamp, which used to surface as an opaque ERROR row instead
#     # of an actual PASS/FAIL. Casting makes this work whether the column is
#     # already timestamp(tz), a bare date, or text/varchar holding a parsable
#     # date/time string — on both supported dialects.
#     is_snowflake = executor.get_executor_type() == "snowflake"
#     cast_type = "timestamp_tz" if is_snowflake else "timestamptz"

#     try:
#         age_hours = await _scalar(executor, f"""
#             SELECT EXTRACT(EPOCH FROM (now() - MAX({column}::{cast_type}))) / 3600.0
#             FROM {table}
#         """)
#     except Exception as exc:
#         return _result(
#             table_name, "FRESHNESS_CHECK", "ERROR",
#             message=f"Column '{timestamp_column}' isn't a valid timestamp: {exc}",
#         )
