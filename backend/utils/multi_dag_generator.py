...
VALID_MULTI_CONNECTORS = {"csv", "excel", "google_sheets", "api", "s3", "postgres", "mysql", "oracle", "mongodb", "snowflake", "salesforce", "hubspot", "zoho"}
...
def validate_source(src: dict, index: int) -> list:
    ...
    if ct in ("csv", "excel"):
        ...
        for field in ("src_my_host", "src_my_db", "src_my_user", "src_my_password"):
            if not _clean(src.get(field)):
                errors.append(f"{label} (mysql): {field} is required and cannot be empty.")
        query_error = _validate_query_shape("mysql", src.get("my_query"))
        if query_error:
            errors.append(f"{label}: {query_error}")
        ...
        for field in ("src_ora_host", "src_ora_db", "src_ora_user", "src_ora_password"):
            if not _clean(src.get(field)):
                errors.append(f"{label} (oracle): {field} is required and cannot be empty.")
        query_error = _validate_query_shape("oracle", src.get("ora_query"))
        if query_error:
            errors.append(f"{label}: {query_error}")

    elif ct == "mongodb":
        if not _clean(src.get("src_mongo_connection_string")):
            for field in ("src_mongo_host", "src_mongo_db"):
                if not _clean(src.get(field)):
                    errors.append(f"{label} (mongodb): {field} is required and cannot be empty.")
        elif not _clean(src.get("src_mongo_db")):
            errors.append(f"{label} (mongodb): src_mongo_db is required and cannot be empty.")
        if not _clean(src.get("mongo_collection")):
            errors.append(f"{label} (mongodb): mongo_collection is required and cannot be empty.")
        ...
        if not _clean(src.get("sf_crm_access_token")) and not (
            _clean(src.get("sf_crm_client_id")) and _clean(src.get("sf_crm_client_secret"))
            and _clean(src.get("sf_crm_username")) and _clean(src.get("sf_crm_password"))
        ):
            errors.append(
                f"{label} (salesforce): provide sf_crm_access_token (+ sf_crm_instance_url), "
                "or sf_crm_client_id/sf_crm_client_secret/sf_crm_username/sf_crm_password."
            )
        if not _clean(src.get("sf_crm_object_name")) and not _clean(src.get("sf_crm_soql_query")):
            errors.append(f"{label} (salesforce): sf_crm_object_name or sf_crm_soql_query is required.")

    elif ct == "hubspot":
        if not _clean(src.get("hs_access_token")):
            errors.append(f"{label} (hubspot): hs_access_token is required and cannot be empty.")

    elif ct == "zoho":
        if not _clean(src.get("zoho_access_token")) and not (
            _clean(src.get("zoho_refresh_token")) and _clean(src.get("zoho_client_id")) and _clean(src.get("zoho_client_secret"))
        ):
            errors.append(
                f"{label} (zoho): provide zoho_access_token, or zoho_refresh_token/zoho_client_id/zoho_client_secret."
            )
        if not _clean(src.get("zoho_module")):
            errors.append(f"{label} (zoho): zoho_module is required and cannot be empty.")
...
def _build_source_entry(src: dict, index: int, table_name: str, sync_mode: str,
    ...
    if ct in ("csv", "excel"):
        ...
        entry["SRC_MY_HOST"]     = _clean(src.get("src_my_host"))
        entry["SRC_MY_DB"]       = _clean(src.get("src_my_db"))
        entry["SRC_MY_USER"]     = _clean(src.get("src_my_user"))
        entry["SRC_MY_PASSWORD"] = _clean(src.get("src_my_password"))
        entry["SRC_MY_PORT"]     = src.get("src_my_port", "3306")
        entry["MY_QUERY"]        = _clean(src.get("my_query"))

    elif ct == "oracle":
        entry["SRC_ORA_HOST"]     = _clean(src.get("src_ora_host"))
        entry["SRC_ORA_DB"]       = _clean(src.get("src_ora_db"))
        entry["SRC_ORA_USER"]     = _clean(src.get("src_ora_user"))
        entry["SRC_ORA_PASSWORD"] = _clean(src.get("src_ora_password"))
        entry["SRC_ORA_PORT"]     = src.get("src_ora_port", "1521")
        entry["ORA_QUERY"]        = _clean(src.get("ora_query"))

    elif ct == "mongodb":
        entry["SRC_MONGO_HOST"]              = _clean(src.get("src_mongo_host"))
        entry["SRC_MONGO_DB"]                = _clean(src.get("src_mongo_db"))
        entry["SRC_MONGO_USER"]              = _clean(src.get("src_mongo_user"))
        entry["SRC_MONGO_PASSWORD"]          = _clean(src.get("src_mongo_password"))
        entry["SRC_MONGO_PORT"]              = src.get("src_mongo_port", "27017")
        entry["SRC_MONGO_CONNECTION_STRING"] = _clean(src.get("src_mongo_connection_string"))
        entry["MONGO_COLLECTION"]            = _clean(src.get("mongo_collection"))
        entry["MONGO_QUERY"]                 = _clean(src.get("mongo_query"))
        ...
        entry["SF_CRM_ACCESS_TOKEN"]   = _clean(src.get("sf_crm_access_token"))
        entry["SF_CRM_INSTANCE_URL"]   = _clean(src.get("sf_crm_instance_url"))
        entry["SF_CRM_LOGIN_URL"]      = src.get("sf_crm_login_url", "https://login.salesforce.com")
        entry["SF_CRM_CLIENT_ID"]      = _clean(src.get("sf_crm_client_id"))
        entry["SF_CRM_CLIENT_SECRET"]  = _clean(src.get("sf_crm_client_secret"))
        entry["SF_CRM_USERNAME"]       = _clean(src.get("sf_crm_username"))
        entry["SF_CRM_PASSWORD"]       = _clean(src.get("sf_crm_password"))
        entry["SF_CRM_SECURITY_TOKEN"] = _clean(src.get("sf_crm_security_token"))
        entry["SF_CRM_OBJECT_NAME"]    = _clean(src.get("sf_crm_object_name"))
        entry["SF_CRM_FIELDS"]         = src.get("sf_crm_fields") or None
        entry["SF_CRM_SOQL_QUERY"]     = _clean(src.get("sf_crm_soql_query"))

    elif ct == "hubspot":
        entry["HS_ACCESS_TOKEN"] = _clean(src.get("hs_access_token"))
        entry["HS_OBJECT_TYPE"]  = src.get("hs_object_type", "contacts")
        entry["HS_PROPERTIES"]   = src.get("hs_properties") or None

    elif ct == "zoho":
        entry["ZOHO_ACCESS_TOKEN"]  = _clean(src.get("zoho_access_token"))
        entry["ZOHO_REFRESH_TOKEN"] = _clean(src.get("zoho_refresh_token"))
        entry["ZOHO_CLIENT_ID"]     = _clean(src.get("zoho_client_id"))
        entry["ZOHO_CLIENT_SECRET"] = _clean(src.get("zoho_client_secret"))
        entry["ZOHO_ACCOUNTS_URL"]  = src.get("zoho_accounts_url", "https://accounts.zoho.com")
        entry["ZOHO_API_DOMAIN"]    = src.get("zoho_api_domain", "https://www.zohoapis.com")
        entry["ZOHO_MODULE"]        = _clean(src.get("zoho_module"))
        entry["ZOHO_FIELDS"]        = src.get("zoho_fields") or None
        entry["ZOHO_CRITERIA"]      = _clean(src.get("zoho_criteria"))
