...
Uses business_context.json from the metadata package.
"""
...
from text_sql.snowflake_config import SNOWFLAKE_CONFIG
import snowflake.connector

from metadata.foreign_keys import get_foreign_keys

# Paths — the metadata package sits alongside text_sql/ under the backend root.
_METADATA_DIR = os.path.join(
    os.path.dirname(os.path.dirname(os.path.abspath(__file__))), "metadata"
)
BUSINESS_CONTEXT_PATH = os.path.join(_METADATA_DIR, "business_context.json")
SCHEMA_CATALOG_PATH = os.path.join(_METADATA_DIR, "schema_catalog.json")
...
class SchemaManager:
    ...
    def load_business_context(self) -> dict:
        """
        Load business_context.json from the metadata package.
        ...
        if os.path.exists(BUSINESS_CONTEXT_PATH):
            ...
            print("Run backend/metadata/generate_business_context.py to generate it.")
    ...
    def extract_foreign_keys(self) -> dict:
        ...
        Uses metadata/foreign_keys.py for extraction.
    ...
    def build_enriched_schema(self) -> Tuple[str, dict]:
        ...
        if cached_hash and current_hash != cached_hash:
            ...
            print("\n   Please run: python backend/metadata/generate_business_context.py")
