...
export const DataQualityBuilder = ({ filePath, connector, onChange }: Props) => {
  ...
  const preview = useQuery({
    queryKey: ["file-preview", filePath, connector],
    enabled: !!filePath,
    queryFn: async () => (await api.get<PreviewResponse>("/preview_file", {
      params: { file_path: filePath, connector },
    })).data,
  });

  const columns = preview.data?.columns ?? [];

  // Reset per-column checks whenever a new file is picked, so stale
  // checks from a previous upload never silently carry over.
  useEffect(() => {
    setChecks({});
    setOpenColumn(null);
  }, [filePath]);
    ...
    <div className="space-y-4">
      ...
      <div className="overflow-hidden rounded-lg border border-border shadow-sm">
        <div className="flex items-center gap-2 border-b border-border bg-muted/50 px-4 py-2.5">
          ...
          <span className="text-sm font-semibold text-foreground">Here's what your file looks like</span>
          {preview.data && (
        ...
        {preview.isError && (
          <div className="flex items-center gap-2 p-5 text-sm text-rose-600 dark:text-rose-400">
            ...
            {(preview.error as any)?.response?.data?.detail ?? "We couldn't preview this file."}
          </div>
        )}
        ...
        {preview.data && (
          <div>
            ...
            <p className="border-t border-border bg-muted/20 px-4 py-2 text-xs text-muted-foreground">
              Showing the first {preview.data.sample_row_count} of {preview.data.total_rows.toLocaleString()} rows — the whole file will be imported.
            </p>
          </div>
        )}
      </div>
