...
// // Per-source table-vs-query mode for Postgres / Snowflake.
// type SourceQueryMode = {
//   pgMode: "table" | "query";
//   pgTable: string;
//   sfMode: "table" | "query";
//   sfTable: string;
...
// const blankQueryMode = (): SourceQueryMode => ({
//   pgMode: "table",
//   pgTable: "",
//   sfMode: "table",
//   sfTable: "",
...
//   const [sourceQueryModes, setSourceQueryModes] = useState<Record<number, SourceQueryMode>>({});
...
//   const getQueryMode = (index: number): SourceQueryMode => sourceQueryModes[index] ?? blankQueryMode();

//   const updateQueryMode = (index: number, patch: Partial<SourceQueryMode>) =>
//     setSourceQueryModes((current) => ({ ...current, [index]: { ...getQueryMode(index), ...patch } }));
...
//     }));
//     updateQueryMode(index, blankQueryMode());
...
//       return next;
//     });
//     setSourceQueryModes((current) => {
//       const next: Record<number, SourceQueryMode> = {};
//       Object.entries(current).forEach(([key, val]) => {
//         const i = Number(key);
//         if (i < index) next[i] = val;
//         else if (i > index) next[i - 1] = val;
...
//         if (qm.pgMode === "table" && !qm.pgTable.trim()) return `${label}: table name is required.`;
//         if (qm.pgMode === "query" && !source.pg_query.trim()) return `${label}: SQL query is required.`;
//       }
//       if (source.connector_type === "snowflake") {
//         if (qm.sfMode === "table" && !qm.sfTable.trim()) return `${label}: table name is required.`;
//         if (qm.sfMode === "query" && !source.sf_query.trim()) return `${label}: SQL query is required.`;
//       }
//       return "";
//     }
...
//     }
//     if (source.connector_type === "postgres") {
//       if (!source.src_pg_host || !source.src_pg_db || !source.src_pg_user) {
...
//     if (source.connector_type === "snowflake") {
//       if (!source.sf_account || !source.sf_user || !source.sf_warehouse || !source.sf_database) {
...
//                                 <input type="radio" name={`pgMode-${index}`} checked={qm.pgMode === "table"} onChange={() => updateQueryMode(index, { pgMode: "table" })} />
//                                 Table name
//                               </label>
//                               <label className="flex items-center gap-1.5 cursor-pointer">
//                                 <input type="radio" name={`pgMode-${index}`} checked={qm.pgMode === "query"} onChange={() => updateQueryMode(index, { pgMode: "query" })} />
//                                 Custom SQL query
//                               </label>
//                             </div>
...
//                                   updateQueryMode(index, { pgTable: v });
...
//                             <div className="flex items-center gap-4 text-sm font-medium text-foreground">
//                               <label className="flex items-center gap-1.5 cursor-pointer">
//                                 <input type="radio" name={`sfMode-${index}`} checked={qm.sfMode === "table"} onChange={() => updateQueryMode(index, { sfMode: "table" })} />
//                                 Table name
//                               </label>
//                               <label className="flex items-center gap-1.5 cursor-pointer">
//                                 <input type="radio" name={`sfMode-${index}`} checked={qm.sfMode === "query"} onChange={() => updateQueryMode(index, { sfMode: "query" })} />
//                                 Custom SQL query
//                               </label>
//                             </div>
...
//                                 onChange={(e) => {
//                                   const v = e.target.value;
//                                   updateQueryMode(index, { sfTable: v });
//                                   updateSource(index, "sf_query", buildSelectQuery(v));
//                                 }}
//                               />
//                             ) : (
//                               <textarea
...
//                         </p>
//                         {source.connector_type === "postgres" && source.pg_query && (
//                           <p className="mt-1 font-mono text-xs text-muted-foreground break-all">{source.pg_query}</p>
//                         )}
//                         {source.connector_type === "snowflake" && source.sf_query && (
//                           <p className="mt-1 font-mono text-xs text-muted-foreground break-all">{source.sf_query}</p>
//                         )}
//                       </div>
//                     );
//                   })}
