...
export const Metrics = () => {
  ...
  return (
    <div className="space-y-5">
      <h2 className="h-section flex items-center gap-2"><ChartNoAxesCombined className="h-5 w-5" /> Metrics</h2>
      <Card>
            ...
            <CardContent>
              <ResponsiveContainer width="100%" height={260}>
                <BarChart data={chartData}>
                  ...
                  <YAxis />
                  <Tooltip />
                  <Bar dataKey="duration" fill="#0891b2" radius={[4, 4, 0, 0]} />
                </BarChart>
              </ResponsiveContainer>
            </CardContent>
          </Card>
              ...
              <table className="w-full text-sm">
                ...
                <tbody>
                  {(summary.data ?? []).map((row: any) => (
                    <tr key={`${row.pipeline_id}-${row.connector_type}`} className="border-b border-slate-100">
                      <td className="py-3 pr-4 font-medium">{row.pipeline_id}</td>
                      <td className="py-3 pr-4">{row.connector_type}</td>
                      <td className="py-3 pr-4">{row.total_runs}</td>
                      <td className="py-3 pr-4">{fmtInt(row.total_rows)}</td>
                      <td className="py-3 pr-4">{row.avg_duration_sec}s</td>
                      <td className="py-3 pr-4"><StatusBadge status={`${row.success_count} success`} /></td>
                      <td className="py-3 pr-4">{row.failed_count}</td>
