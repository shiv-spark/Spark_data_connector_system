...
export const DashboardEditor = () => {
  ...
  const command = useMutation({
    mutationFn: async ({ message, slot }: { message: string; slot?: number }) => {
      const r = await api.post(`/agent/dashboard/${dashboardId}/command`, { message, slot });
      return r.data as {
        status: string;
        reply?: string;
...
const ChartCanvasMount = ({ html, className }: { html: string; className?: string }) => {
  const ref = useRef<HTMLDivElement>(null);
  const instanceIdRef = useRef(`m${Math.random().toString(36).slice(2)}`);

  useEffect(() => {
    const el = ref.current;
    if (!el) return;

    // Plotly's fig.to_html() bakes a fixed UUID into both the container
    // div's id and the Plotly.newPlot(...) call inside the script tag.
    // If the SAME html string is mounted in two places at once (card +
    // fullscreen expand), the ids collide and Plotly only draws into one
    // of them — the other stays blank. Make the id unique per mount.
    const idMatch = html.match(/id="([a-f0-9-]{20,})"/);
    let scopedHtml = html;
    if (idMatch) {
      const originalId = idMatch[1];
      scopedHtml = html.split(originalId).join(`${originalId}-${instanceIdRef.current}`);
    }

    el.innerHTML = scopedHtml;
    const scripts = Array.from(el.querySelectorAll("script"));
    scripts.forEach((old) => {
      const next = document.createElement("script");
      Array.from(old.attributes).forEach((a) => next.setAttribute(a.name, a.value));
    ...
    <>
      ...
      {expanded && (
        ...
        >
          ...
          >
            <div className="relative z-20 flex items-center justify-between border-b border-border bg-card px-4 py-3">
              <div className="min-w-0">
